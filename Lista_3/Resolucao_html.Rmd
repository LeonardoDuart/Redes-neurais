...
n <- 1000
x1 <- seq(-7, 7, length.out=n)
x2 <- seq(-7, 7, length.out=n)
...
ggplot(., aes(x1, x2, z = z)) +
  geom_contour_filled(breaks = seq(-250, 170, by = 35)) +
...
```{r 1e,  error=FALSE, cache=TRUE, eval=FALSE}
...
for(i in 1:(n_pontos*num_passos)){
    ...
    base$gradiente[i] <- funcao(pontos)
}
```
...
**h)** Repita o item d), substituindo o método do gradiente pelo método RMSProp (veja a Seção 8.5.2do livro *Deep Learning*). Use taxa de aprendizado $\epsilon = 0.001$, taxa de decaimento $\rho = 0.9$ e constante $\delta = 10^{−6}$.
